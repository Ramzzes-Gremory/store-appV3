<template>
    <nuxt-link 
        ...
        :class="[isActive ? 'filter-blue' : '']">
    <li  class="flex justify-start items-center  leading-8 text-base list-image-none py-4">
        <Icon 
            :name="icon"    
            class="bg-gray-500 text-2xl whitespace-nowrap flex-shrink-0"  
            style=""
            /> 

        <div class="grid transition=[grid-template-columns, opacity] duration-500 ease-in-out" :class="asideMenu.isSidebarOpen ? 'grid-cols-[1fr] opacity-100 ml-4' : 'grid-cols-[0fr] opacity-0 ml-0'">
